...
// GET /presets – with filters & pagination
...
// Upload preset – status now 'pending'
...
// Download preset (no change, kept as is)
router.post('/:id/download', auth, async (req, res) => {
  ...
  return res.status(404).json({
    error: 'Preset file is missing on server. Please contact support or re-upload the preset.'
  });
});

// Delete preset (no change)
router.delete('/:id', auth, async (req, res) => {
  ...
  if (preset.fileUrl) {
    const projectRoot = path.join(__dirname, '../..');
    const filePath = path.join(projectRoot, preset.fileUrl);
    if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
  }
  if (preset.previewImage) {
    const projectRoot = path.join(__dirname, '../..');
    const previewPath = path.join(projectRoot, preset.previewImage);
