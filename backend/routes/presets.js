...
router.post('/:id/download', auth, async (req, res) => {
  ...
  if (filePath && fs.existsSync(filePath)) {
    ...
    return res.download(filePath, downloadName, (err) => { if (err) console.error('Download error:', err); });
  }
...
router.post('/:id/ad-impression', auth, async (req, res) => {
  ...
  const preset = db.data.presets.find(p => p.id === req.params.id);
  if (!preset) return res.status(404).json({ error: 'Preset not found' });
  if (preset.authorId === req.user.id) return res.json({ success: true, message: 'Author view not counted' });
...
router.post('/:id/like', auth, async (req, res) => {
  ...
  if (idx === -1) {
    ...
    if (preset.authorId !== userId) {
      await createNotification(preset.authorId, 'like', `${req.user.name} liked your preset "${preset.name}"`, `/preset/${preset.id}`);
    }
  } else {
