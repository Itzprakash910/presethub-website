...
router.post('/:id/follow', auth, async (req, res) => {
  ...
  if (isFollowing) {
    ...
    await db.write();
    return res.json({ following: false, followersCount: target.followers.length, followingCount: current.following.length });
  } else {
    ...
    return res.json({ following: true, followersCount: target.followers.length, followingCount: current.following.length });
  }
});
...
router.post('/referrals/generate', auth, async (req, res) => {
  ...
  const user = db.data.users.find(u => u.id === req.user.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  if (!user.referral) user.referral = { code: null, referredBy: null, referralCount: 0, referralRewardDays: 0 };
